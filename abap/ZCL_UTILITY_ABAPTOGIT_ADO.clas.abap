...
CLASS zcl_utility_abaptogit_ado IMPLEMENTATION.
  ...
  METHOD http_get_json.
    ...
    lv_success = me->create_http_client(
        EXPORTING
            iv_url = lv_url
    ...
    IF lv_success = abap_false.
      me->write_telemetry( iv_message = |HTTP_GET_JSON HTTP GET URL not found for { lv_url }| ).
      ev_status = 404.
      RETURN.
    ENDIF.
  ...
  METHOD http_post_json_text.
    ...
    IF lv_success = abap_false.
      me->write_telemetry( iv_message = |HTTP_POST_JSON HTTP POST URL not found for { lv_url }| ).
      ev_status = 404.
      RETURN.
    ENDIF.
